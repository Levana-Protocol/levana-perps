...
impl FarmingEmissions {
    pub fn new_strategy() -> impl Strategy<Value = Self> {
        (100.0f32..1000000.0f32).prop_flat_map(|emissions_amount| {
            (10u32..100000u32).prop_flat_map(move |emissions_duration_seconds| {
                let action_builders: Vec<_> = (0..=10)
                    .map(|_| ActionBuilder::new_strategy(emissions_duration_seconds))
                    .collect();
                action_builders.prop_map(move |mut action_builders| {
                    ...
                    for action_builder in action_builders {
                        let collateral = match action_builder.kind {
                            ...
                            ActionBuilderKind::WithdrawPerc(perc) => {
                                ...
                                let collateral = Collateral::from_decimal256(
                                    current_deposit.into_decimal256() * perc,
                                );
                                sanitize_collateral(collateral)
                            }
                        };
                    ...
                    Self {
                        ...
                        emissions_amount: emissions_amount.to_string().parse().unwrap(),
                    }
                })
            })
        })
...
fn sanitize_collateral(collateral: Collateral) -> Result<Collateral> {
    ...
    let collateral = token
        .into_u128(collateral.into_decimal256())?
        .ok_or_else(|| anyhow!("invalid collateral"))?;
...
impl ActionBuilder {
    pub fn new_deposit_strategy() -> impl Strategy<Value = ActionBuilderKind> {
        (1u32..100000u32).prop_map(|collateral| {
            ActionBuilderKind::Deposit(Collateral::from_decimal256(Decimal256::from_ratio(
                collateral, 1u32,
            )))
        })
    }
    pub fn new_withdraw_strategy() -> impl Strategy<Value = ActionBuilderKind> {
        (0.1f32..0.9f32).prop_map(ActionBuilderKind::WithdrawPerc)
    }
