...
#[derive(Debug)]
pub struct ArraySerializedVector(pub Vec3);
...
impl Polyhedron {
    ...
    pub fn new_isocahedron(radius: f32, detail: u32) -> Polyhedron {
        ...
        base_isocahedron.add_position(Vec3::new(-1.0, t, 0.0));
        base_isocahedron.add_position(Vec3::new(1.0, t, 0.0));
        base_isocahedron.add_position(Vec3::new(-1.0, -t, 0.0));
        base_isocahedron.add_position(Vec3::new(1.0, -t, 0.0));
        base_isocahedron.add_position(Vec3::new(0.0, -1.0, t));
        base_isocahedron.add_position(Vec3::new(0.0, 1.0, t));
        base_isocahedron.add_position(Vec3::new(0.0, -1.0, -t));
        base_isocahedron.add_position(Vec3::new(0.0, 1.0, -t));
        base_isocahedron.add_position(Vec3::new(t, 0.0, -1.0));
        base_isocahedron.add_position(Vec3::new(t, 0.0, 1.0));
        base_isocahedron.add_position(Vec3::new(-t, 0.0, -1.0));
        base_isocahedron.add_position(Vec3::new(-t, 0.0, 1.0));
    ...
    fn subdivide_triangle(&mut self, a: Vec3, b: Vec3, c: Vec3, radius: f32, detail: u32) {
        let cols = 2usize.pow(detail);
        let mut new_vertices: Vec<Vec<Vec3>> = vec![];
    ...
    fn add_position(&mut self, vertex: Vec3) -> usize {
        ...
        if let Some(added_vert_index) = self.added_vert_cache.get(&vertex_key) {
            ...
            self.normals
                .push(ArraySerializedVector(Vec3::new(0.0, 0.0, 0.0)));
            self.colors
                .push(ArraySerializedVector(Vec3::new(1.0, 1.0, 1.0)));
    ...
    fn truncated(&mut self, other: Polyhedron) {
        ...
        let mut mid_centroid_cache: HashMap<(usize, usize, usize), Vec3> = HashMap::new();
        let mut hex_count = 0;
    ...
    fn triangle_centroids(&self) -> HashMap<usize, Vec3> {
        let mut triangle_centroids: HashMap<usize, Vec3> = HashMap::new();
        for i in 0..self.cells.len() {
    ...
    fn calculate_mid_centroid(
        ...
        centroid: Vec3,
        triangle_centroids: &HashMap<usize, Vec3>,
        mid_centroid_cache: &mut HashMap<(usize, usize, usize), Vec3>,
    ) -> Vec3 {
    ...
    pub fn assign_random_face_colors(&mut self) {
        ...
        for i in 0..self.faces.len() {
            let face_color = Vec3::new(rng.gen(), rng.gen(), rng.gen());
...
fn calculate_centroid(pa: Vec3, pb: Vec3, pc: Vec3) -> Vec3 {
...
fn find_center_of_triangles(
    ...
    let mut center_point: Vec3 = Vec3::new(0.0, 0.0, 0.0);
