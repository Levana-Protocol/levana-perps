...
use perps_exes::{
    config::{
        load_config, ChainConfig, ConfigTestnet, DeploymentInfo, GasAmount, GasDecimals,
        LiquidityConfig, LiquidityTransactionConfig, TraderConfig, UtilizationConfig,
        WatcherConfig,
    },
    prelude::*,
    PerpsNetwork,
...
impl Opt {
    ...
    fn get_bot_config_testnet(
        ...
        let config_testnet = ConfigTestnet::load(testnet.config_testnet.as_deref())?;
        let DeploymentInfo {
            config,
            network,
            wallet_phrase_name,
        } = config_testnet.get_deployment_info(&testnet.deployment)?;
        let ChainConfig {
            tracker,
            faucet,
            spot_price: _,
        ...
        } = ChainConfig::load(testnet.config_chain.as_deref(), network)?;
        ...
        let testnet = BotConfigTestnet {
            ...
            faucet,
            price_api: config_testnet.price_api.clone(),
            ...
            ultra_crank_wallets: (0..config.ultra_crank)
                .map(|index| {
                ...
                .collect::<Result<_>>()?,
            liquidity_config: if config.liquidity {
                Some(config_testnet.liquidity.clone())
            } else {
                None
            },
            utilization_config: if config.utilization {
                Some(config_testnet.utilization)
            } else {
                None
            },
            trader_config: Some((
                testnet.traders.unwrap_or(config.traders),
                config_testnet.trader,
            )),
            rpc_nodes: match &self.rpc_url {
                None => rpc_nodes.iter().map(|x| Arc::new(x.clone())).collect(),
                Some(rpc) => vec![rpc.clone().into()],
            },
            ignore_stale: config.ignore_stale,
            seconds_till_ultra: config.seconds_till_ultra,
        ...
        let config = BotConfig {
            ...
            needs_price_update_params: NeedsPriceUpdateParams {
                on_chain_publish_time_age_threshold: chrono::Duration::seconds(
                    config.max_price_age_secs.into(),
                ),
                on_off_chain_price_delta: config.max_allowed_price_delta,
            },
            gas_decimals,
            http_timeout_seconds,
            min_gas: config.min_gas,
            min_gas_high_gas_wallet: config.min_gas_high_gas_wallet,
    ...
    fn get_bot_config_mainnet(
        ...
        let watcher = load_config::<WatcherConfig>(watcher_config, "LEVANA_WATCHER")?;
        let gas_decimals = ChainConfig::load(None, *network)?.gas_decimals;
        Ok(BotConfig {
